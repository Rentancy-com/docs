...
| 3 | **OAuth 2.0 client credentials** | Server-to-server integrations | `LOFTY_CLIENT_ID` + `LOFTY_CLIENT_SECRET` + `LOFTY_CUSTOMER_KEY` (resource owner) |
...
Use this for server-to-server scenarios where there is no human to complete a browser flow. This implements the [OAuth 2.0 Client Credentials Grant](https://datatracker.ietf.org/doc/html/rfc6749#section-4.4) with an additional resource owner identifier.
...
### Step 3 -- specify the target account
