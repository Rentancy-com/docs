...
| --- | --- | --- | --- |
| 1 | **Token URL** | CI/CD, automation that already has a session ID | `LOFTY_TOKEN_URL` + `LOFTY_SESSION_ID` |
| 2 | **Browser OAuth (PKCE)** | Interactive use on a developer machine | `lofty-cli auth login-browser` |
| 3 | **OAuth 2.0 client credentials** | Server-to-server integrations | `LOFTY_CLIENT_ID` + `LOFTY_CLIENT_SECRET` + `LOFTY_CUSTOMER_KEY` |
| 4 | **Direct access token** | One-off testing, container env injection | `LOFTY_ACCESS_TOKEN` (+ optional `LOFTY_REFRESH_TOKEN`) |

<Note>
  Tokens obtained by methods 1, 2, and 3 are cached locally and refreshed automatically when they expire.
</Note>

## Method 1 -- Token URL (highest priority)
...
You can also pass the values inline, which overrides the environment variables for that invocation:
...
## Method 2 -- Browser OAuth (recommended for humans)

The simplest method when you are working from a laptop. The CLI launches your default browser, you log in once, and the resulting token is stored locally.
...
Use this for server-to-server scenarios where there is no human to complete a browser flow.

### Step 1 -- register a vendor application

Sign in to the [Lofty Vendor Portal](https://api.lofty.com/vendor/frontend/static/index.html#/login) to obtain a `client_id` and `client_secret`.

### Step 2 -- export the vendor credentials
...
### Step 3 -- provide the customer key

The customer key represents the Lofty user whose data you are accessing. Find it in the Lofty CRM under **Settings > Integrations > API**.
