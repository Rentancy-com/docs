...
<Warning>
All IDs returned by the API (`leadId`, `assignedUserId`, etc.) are **64-bit integers** sent as JSON numbers. JavaScript's `JSON.parse()` will silently corrupt values exceeding `2^53 - 1`. Use string-based parsing in JS/TS clients — see [JS / TS Integration](/javascript-integration).
</Warning>
