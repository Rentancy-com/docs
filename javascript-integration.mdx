---
title: "JavaScript and TypeScript Integration Guide"
...
All entity IDs returned by the Lofty API are 64-bit integers. JavaScript's `Number` type uses IEEE 754 double-precision floating point, which can only safely represent integers up to 2^53-1 (9,007,199,254,740,991). Lofty IDs can exceed this range. When they do, JavaScript silently rounds the value to the nearest representable number — a different integer — without throwing an error or warning. Your application will store, compare, and transmit the wrong ID.

## The problem

When you call `JSON.parse()` on a response body containing a large integer ID, the parser converts it to a JavaScript `Number`. If the value exceeds `Number.MAX_SAFE_INTEGER`, it gets corrupted:
...
console.log(lead.id); // 563172647619608 may be silently rounded to a different value
```

The original JSON from the server might contain `563172647619623`, but after `response.json()` parses it, you could be holding `563172647619608` — an ID that points to a different lead or doesn't exist at all. There is no exception, no warning, and no indication that the value changed.
